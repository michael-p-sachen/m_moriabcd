...
import { createInitialDocumentsState, documentsReducer, type DocumentsAction, type DocumentsState } from '../reducers';
...
export const useDocumentsNavigation = () => {
  ...
  const transitionTokenRef = useRef(1);
  ...
  // Mutable refs so async callbacks always read the latest values without stale closures.
  // Synced in useLayoutEffect (not during render) to satisfy concurrent-mode constraints.
